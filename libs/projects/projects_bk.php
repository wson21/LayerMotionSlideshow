...
add_action( 'init', 'uep_add_pcategory' );

function uep_add_pcategory() {
    $labels = array(
...
//add_action( 'init', 'uep_add_clients' );

function uep_add_clients() {
    $labels = array(
        'name'              => 'clients',
...
add_action( 'init', 'uep_add_type' );

function uep_add_type() {
    $labels = array(
...
<div id="postexcerpt" class="postbox closed" style="display: block;">
	<button type="button" class="handlediv button-link" aria-expanded="false">
    	<span class="screen-reader-text">Toggle panel: Slideshow</span><span class="toggle-indicator" aria-hidden="true"></span>
    </button>
		...
		<?php
			...
			if ($attachments) {
				foreach ($attachments as $attachment_id) {
					...
					echo '<li class="image" data-attachment_id="'.$attachment_id.'"> <img src="'. $image_attributes[0] .' " class"image1" alt="<?php echo $thumb_title;?>"/>
					
						<span><a href="#" class="delete_slide" title="'.__( 'Delete image', 'quitenicebooking').'">'.__('Delete', 'quitenicebooking').'</a></span>
					</li>';
...
<div id="postexcerpt" class="postbox closed" style="display: block;">
	<button type="button" class="handlediv button-link" aria-expanded="false">
    	<span class="screen-reader-text">Toggle panel: Location</span><span class="toggle-indicator" aria-hidden="true"></span>
    </button>
    ...
    <div class="inside">
		...
		<input class="widefat" id="_location" type="text" name="_location" placeholder="exp: http://www.belleview.com" value="<?php echo $location; ?>" />
	</div>
</div>
</div>
...
<?php /*?><img src="<?php echo wp_get_attachment_url( get_post_thumbnail_id($id) ); ?>" width="150"/><?php */?>

<?php //echo $thumb_id = get_post_thumbnail_id($id) ?> <br/>

<?php //echo wp_get_attachment_url($thumb_id); ?>
...
<?php /*?><?php
		...
		if ( $attachments ) {
			foreach ( $attachments as $attachment ) {
				...
				$thumb_title = $attachment->post_title;?>
                
                <?php $thumb_title;?>
                <?php $thumb_url;?>
				
				<img src="<?php echo $thumb_url;?>" width="100" alt="<?php echo $thumb_title;?>"/>
...
function save_wpse_1( $post_id, $post_object )
{
	...
	//get_post_meta($post->ID, '_slideshow_images', true);
	...
	switch( $column ) {
		...
		case 'thumb' :
			...
			if( $image_attributes ) {?>
                ...
                <?php /*?><img src="<?php echo $image_attributes[0]; ?>" width="<?php echo $image_attributes[1]; ?>" height="<?php echo $image_attributes[2]; ?>"><?php */?>
